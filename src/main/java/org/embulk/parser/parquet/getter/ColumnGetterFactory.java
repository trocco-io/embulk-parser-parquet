...
public class ColumnGetterFactory {
    final private Schema parquetSchema;
    final private PageBuilder pageBuilder;
    final private TimestampFormatter[] timestampFormatters;
    final private TimestampUnit[] timestampUnits;

    public ColumnGetterFactory(Schema parquetSchema, PageBuilder pageBuilder, TimestampFormatter[] timestampFormatters, TimestampUnit[] timestampUnits)
    {
    ...
    public BaseColumnGetter newColumnGetter(Column column)
    {
        ...
        if(fieldSchema.getType() == Schema.Type.UNION) {
            Schema schema = fieldSchema.getTypes().stream().filter(x -> x.getType() != Schema.Type.NULL).findFirst().orElse(null);
            if(schema == null) {
    ...
    private BaseColumnGetter getColumnGetterFromTypeName(Schema.Type type, TimestampFormatter timestampFormatter, TimestampUnit timestampUnit)
    {
        switch (type) {
            case STRING:
            case ENUM:
